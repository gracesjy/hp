...
<br>
이것은 많은 시간을 걸려서 만든 쉽게 찾아볼 수 없는 제품이다.  Oracle Database 7.x 부터 사용했었고, 대부분의 프로젝트를 Oracle Database 와
함께 했었다.  PL/SQL, Function, Pro*C 등을 해 보았지만 이것 말고도 Oracle Database 는 마지막 퍼즐을 제공한다. <br>
이것을 아는 이는 한국에는 거의 없으며 이 기반으로 개발을 하는 것은 전 세계적으로도 손으로 꼽을 정도로 적다. <br>
이 BDAE 는 Python, R, Java 등의 VM 을 직접 로딩하고, 이 데이터를 Oracle Database 가 이해할 수 있는 데이터로 변환하는 작업을 자동으로 하도록
되어 있다. 이 부분은 SQL 보다 하단에서 이뤄진다. <br>
그래서 DBA 도, SQL 잘 만드는 개발자들도 전혀 도움이 되지 않고 무엇보다 이런 것들을 전혀 모른다는 것이다. <br>
게다가 Oracle 은 공식적으로 이 관련한 문서나 예제들을 전혀 제공하지 않는다. <br>
<br>
