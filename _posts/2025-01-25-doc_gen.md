...
외부 테이블 데이터는 {TableName.columnName} 으로 넣으면 되고 현재에는 동일한 Context 로 해당 Row 들이 표에 수직의 경우 넣어진다.<br>
수평 병합 상태이면서 수직으로 Generation 되는 경우에는 Set HV 가 된다. <br>
...
### ✅ 후기
예전에 프로젝트를 할 때 이런 것들에 대한 제안을 한 적이 있었고 프로토타입을 하였었다.  Report 툴은 Label 이 일정했을 때 가능하지만, 그 당시의 표는
비정형 표였으며, 상단 또는 좌측 라벨을 기준으로 레코드를 단순히 보여주는 단순한 형태가 아니었다. 따라서 표의 셀(Cell) 에 대한 정보를 가지고 있어야 했다.
<br>
