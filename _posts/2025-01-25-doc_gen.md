...
문제는 표이다.  일반적인 표의 작성을 이야기 하는 것은 아니다.  비정형 표에 대해서 이야기 하는 것이고, 이는 MS Word 를 사용해도 표 작성은 쉽지가 않다. <br>
본 토픽은 표에 대한 병합에 대한 것이다.  기존 셀을 Split 하는 개념보다는 전체의 표의 Matrix 를 가지고 병합해 가는 방법이다. <br>

## ✅ Designer
이 소스는 Copilot 과 ChatGPT 의 도움을 받았다.  Vue.js 는 해 보았지만 React 는 처음이었다. 목적을 이야기 하니 AI 는 자연스럽게 React 소스를 처음 넘겨 주었고,<br>
하는 김에 React 를 배워서 해야 했는데, 드래그로 병합을 하기 위한 것이고 템플릿을 DB 화 하고 이를 기반으로 백엔드에서 Generation 하는 목적이다.<br>
